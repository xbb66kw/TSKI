...
###########################################################################
# You may encounter a problem of not finding Fortran in the R path.
# In my case, R reports '/usr/local/lib/libgfortran.5.dylib' cannot be found.
# By the terminal command
#
# $ locate libgfortran.5.dylib
#
# The libgfortran.5.dylib file is located at
# /usr/local/Cellar/gcc/12.2.0/lib/gcc/12/libgfortran.5.dylib
# and perhaps many other locations.
# Then I use the terminal comman
#
# $ ln /usr/local/Cellar/gcc/12.2.0/lib/gcc/12/libgfortran.5.dylib /usr/local/lib/libgfortran.5.dylib
#
# Then it's all set.
#
# Also, read this post on Stack Overflow:
...
isee.X <- function(X, permu, reg.fac, sis.use = 0, bia.cor = 0){
  ...
  stan_X = scale(X, center=F, scale=TRUE)/sqrt(n-1)
...
slasso <- function(X, y, lam, betap, windows.use = 1, maxite = 50, tol = 1e-2){
  ...
  nc = ncol(X)
  if (windows.use) {
    # paste0(getwd(), "/R/RANK-code/slasso.dll")
    dyn.load(here("R", "RANK-code", "slasso.dll"))
  } else {
    # paste0(getwd(), "/R/RANK-code/slasso.so")
    dyn.load(here("R", "RANK-code", "slasso.so"))
    #
  }
...
beta.block <- function(X, block.ind, lambda, reg.fac, windows.use, sis.use){
  ...
  for (j in 1:block.size){
    if (sis.use) {
      ...
      set0<-order(abs(cor(X[, -block.ind], X[, block.ind[j]])),decreasing=TRUE)
...
# An additional function for producing the augmented design matrix with knockoffs

gknockoffX <- function(X, Omega){
  n = nrow(X)
  p = ncol(X)

  # Precision matrix (Omega = inverse covariance matrix)
